...
# === QUICK VIEW DROPDOWN ABOVE MAPS ===
quick_view = st.selectbox(
    "Quick View (big changes):",
    [""] + list(QUICK_VIEWS.keys()),
    index=0  # blank default
)

# If a quick view is selected, show description & apply filters
if quick_view:
    view_config = QUICK_VIEWS[quick_view]

    # Show description box above maps
    st.info(f"**Changes in this view:** {view_config['desc']}")

    # Apply filters automatically for both region and turf filters
    selected_regions_raw = view_config['regions']
    selected_turfs = view_config['turfs']
else:
    # Blank defaults when nothing selected
    selected_regions_raw = []
    selected_turfs = []

# === YOUR MAP DRAWING CODE GOES BELOW ===
...
# -----------------------------
# Sidebar: Quick views + shared filters
# -----------------------------
with st.sidebar:
    st.header("Quick views")
    ...
    QUICK_VIEWS = {
        ...
        "R07 changes": {
            ...
            "turfs": [
                "R07F - Placeholder3",
                "R07D - Placeholder2",
                "R11E - Colonial Heights / Chesterfield",
                "R07E - North Chesterfield",
            ],
        },
        "R09 split": {
            "regions": ["R09 - Suffolk"],
    ...
    # Apply quick view by setting session_state before widgets render
    qv_choice = st.selectbox("Select a quick view", list(QUICK_VIEWS.keys()), index=0, key="quick_view")
    if qv_choice != "—" and QUICK_VIEWS[qv_choice]:
        st.session_state["regions_ms"] = QUICK_VIEWS[qv_choice]["regions"]
        st.session_state["turfs_ms"]   = QUICK_VIEWS[qv_choice]["turfs"]
    ...
    # Multiselects use keys so we can prefill via session_state from quick view
    default_regions = st.session_state.get("regions_ms", ["VA All Regions"])
    ...
    default_turfs = st.session_state.get("turfs_ms", [])
    selected_turfs = st.multiselect(
        "Select Turf(s):",
        turfs_all,
        default=default_turfs,
        key="turfs_ms"
    )
    ...
    if len(df_filt) > 0:
        ...
        if filtered_features:
            ...
            folium.GeoJson(
                ...
                tooltip=folium.GeoJsonTooltip(   # ALWAYS ON now
                    fields=[
